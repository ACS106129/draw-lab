...
namespace draw {
	Command::Command(const std::string& input) {
		...
		else if (tryEmplaceKeyword(input, "ambient", 3))
			type = CommandType::AMBIENT;
		else if (tryEmplaceKeyword(input, "background", 3))
		...
		else if (tryEmplaceKeyword(input, "light", 7))
			type = CommandType::LIGHT;
		else if (tryEmplaceKeyword(input, "object", 7))
