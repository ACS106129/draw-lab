...
namespace draw {
	...
	const std::vector<std::vector<Point>> Blueprint::buildPolygons(const TransformMatrix& tm) const {
		std::vector<Point> transVertices;
		std::vector<std::vector<Point>> polygons;
		for (const auto& v : vertices)
			transVertices.emplace_back(tm * *v);
		for (const auto& f : faces) {
			std::vector<Point> temps{ transVertices[(int)f->x - 1], transVertices[(int)f->y - 1], transVertices[(int)f->z - 1] };
			if (f->w >= 1)
				temps.emplace_back(transVertices[(int)f->w - 1]);
			polygons.emplace_back(temps);
		}
		return polygons;
	}
